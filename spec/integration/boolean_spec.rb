...
# The view and controller helpers are basically identical, so we use the same set of tests for both.
# The only thing that changes between the two are the routes we use to access them.

%w(view controller).each do |type|
  describe "A boolean a/b test as a #{type} helper" do

    # Trigger the a/b test, and return the true/false it chose.
    define_method :participate! do
      get "/#{type}_tests/test/boolean"
      eval(last_response.body.strip)
    end

    # Trigger the conversion.
    define_method :convert! do
      get "/#{type}_tests/test/boolean_bingo"
    end
